...
def verify_core_manifest_tree(core_root: Path) -> dict:
    """Strictly validate an unpacked CORE handoff against its own manifest."""
    ...
    return {
        "status": "PASS_MODEL_INPUTS_WITH_PACKAGING_EXCEPTION"
        if not mismatched and not disallowed_missing else "FAIL",
...
def prepare_inputs(root: Path, cfg: dict, force: bool = False) -> PreparedInputs:
    ...
    core_value = cfg["inputs"]["core_zip"]
    core_candidate = Path(core_value)
