...
def verify_core_manifest_tree(core_root: Path) -> dict:
    ...
    checked = 0; verified = 0; missing: list[str] = []; mismatched: list[dict[str, str | int]] = []
    with manifest.open("r", encoding="utf-8-sig", newline="") as handle:
        for row in csv.DictReader(handle):
            rel = str(row["relative_path"]).replace("\\", "/")
            path = core_root / Path(rel); checked += 1
            if not path.exists():
                missing.append(rel); continue
            actual_size = path.stat().st_size; actual_sha = sha256_file(path)
            if actual_size != int(row["size_bytes"]) or actual_sha.lower() != str(row["sha256"]).lower():
                mismatched.append({"relative_path": rel, "expected_sha256": str(row["sha256"]), "actual_sha256": actual_sha, "expected_size": int(row["size_bytes"]), "actual_size": actual_size})
            else:
                verified += 1
    ...
    mismatch_payload = "\n".join(f"{row['relative_path']}|{row['expected_size']}|{row['actual_size']}|{row['expected_sha256']}|{row['actual_sha256']}" for row in sorted(mismatched, key=lambda value: str(value["relative_path"])))
    return {
        "status": "PASS_MODEL_INPUTS_WITH_PACKAGING_EXCEPTION" if not mismatched and not disallowed_missing else "FAIL",
        "manifest": str(manifest), "manifest_sha256": sha256_file(manifest), "manifest_rows": checked,
        "verified_files": verified, "missing_files": missing, "allowed_missing_non_model_files": allowed_missing,
        "disallowed_missing_files": disallowed_missing, "mismatched_files": mismatched,
...
def prepare_inputs(root: Path, cfg: dict, force: bool = False) -> PreparedInputs:
    ...
    if not core_candidate.is_absolute(): core_candidate = root / core_candidate
    manifest_verification: dict | None = None
    if core_candidate.exists():
        core_zip = core_candidate.resolve(); core_extract = extract_zip_cached(core_zip, cache, force=force)
        integration_root = find_integration_root(core_extract); core_sha256 = sha256_file(core_zip); core_source_mode = "ZIP"
    else:
        ...
        if not fallback.is_absolute(): fallback = root / fallback
        fallback = fallback.resolve(); manifest_verification = verify_core_manifest_tree(fallback)
        ...
        core_zip = core_candidate.resolve(); integration_root = find_integration_root(fallback)
        core_sha256 = "CANONICAL_ZIP_UNAVAILABLE"; core_source_mode = "MANIFEST_VERIFIED_EXTRACTED_TREE"
