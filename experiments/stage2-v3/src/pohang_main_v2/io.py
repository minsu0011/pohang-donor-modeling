...
def verify_extracted_core_tree(root: Path) -> dict:
    ...
    for row in rows:
        ...
        if actual_size != expected_size or actual_sha.lower() != expected_sha:
            mismatched.append({
                "relative_path": relative, "expected_size": expected_size, "actual_size": actual_size,
    ...
    result = {
        "status": status, "manifest": str(manifest.resolve()), "manifest_sha256": _sha256_path(manifest),
        "manifest_rows": len(rows), "verified_files": verified, "missing_files": missing,
        "allowed_missing_non_model_files": allowed_missing, "disallowed_missing_files": disallowed_missing,
